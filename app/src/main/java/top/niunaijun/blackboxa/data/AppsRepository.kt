            ...
            synchronized(mInstalledList) {
                ...
                for (installedApplication in installedApplications) {
                    try {
                        ...
                        // Include pre-installed / system apps too (Gmail, Chrome, Maps, YouTube…),
                        // NOT just user-installed ones — but only ones the user can actually open
                        // (they have a launcher). This skips the hundreds of invisible framework /
                        // service packages that have no UI and can't be cloned meaningfully.
                        if ((installedApplication.flags and ApplicationInfo.FLAG_SYSTEM) != 0) {
                            val launchable = try {
                                BlackBoxCore.getPackageManager()
                                        .getLaunchIntentForPackage(installedApplication.packageName) != null
                            } catch (e: Exception) {
                                false
                            }
