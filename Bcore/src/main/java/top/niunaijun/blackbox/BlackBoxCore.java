...
public class BlackBoxCore extends ClientConfiguration {
    ...
    public void doAttachBaseContext(Context context,
        ...
        if (processName.equals(BlackBoxCore.getHostPkg())) {
            ...
            // startLogcat() disabled: it runs `logcat` which pops the Android 13 "Allow app to
            // access all device logs?" dialog on every launch (annoying, and a needless debug dump).
        ...
        if (isServerProcess()) {
            // Background push keep-alive: keep the daemon up + re-ensure GMS for opted-in clones.
            try {
                top.niunaijun.blackbox.core.GuestKeepAlive.start();
            } catch (Throwable t) {
                Slog.w(TAG, "GuestKeepAlive start failed: " + t.getMessage());
            }
