...
public class DaemonService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            // NOTE: do NOT start DaemonInnerService here. That was an old pre-Oreo hack
            // to HIDE the foreground notification by cancelling it — but on Android 8+
            // cancelling the foreground notification demotes the service, making the
            // whole container (and cloned apps' push services) killable. Keeping the
            // ongoing notification is what keeps everything alive in the background.
            if (BuildCompat.isOreo()) {
