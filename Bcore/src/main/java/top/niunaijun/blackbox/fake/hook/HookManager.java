...
public class HookManager {
    ...
    void injectAll() {
        // Per-variant hook installation order. The injectors are independent (they were already
        // stored in an unordered HashMap, so the code tolerates any order), but permuting the
        // order deterministically per build variant means different app variants don't install
        // their hooks in an identical, fingerprintable sequence. Seed is derived from the host
        // package name, so it's stable for a given variant and different across variants.
        java.util.List<IInjectHook> order = new java.util.ArrayList<>(mInjectors.values());
        try {
            long seed = top.niunaijun.blackbox.core.VariantConfig.hookSeed();
            java.util.Collections.shuffle(order, new java.util.Random(seed));
        } catch (Throwable ignored) {
            // fall back to natural order if anything goes wrong
        }
        for (IInjectHook value : order) {
            try {
