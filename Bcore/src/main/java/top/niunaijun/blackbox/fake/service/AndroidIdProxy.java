...
public class AndroidIdProxy extends ClassInvocationStub {
    ...
    private static boolean isAndroidIdKey(Object[] args) {
        if (args != null && args.length > 0 && args[0] instanceof String) {
            String key = (String) args[0];
            return key.contains("android_id") || key.contains("ANDROID_ID")
                    || key.contains("secure_id") || key.contains("device_id");
        }
        return false;
    }

    /** This clone's android_id, or null if no profile is applied (host). */
    private static String cloneAndroidId() {
        DeviceProfile p = DeviceProfile.CURRENT;
        return p != null ? p.androidId : null;
    }
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            try {
                ...
                if (isAndroidIdKey(args) && (result == null || "0".equals(result.toString()) || "".equals(result.toString()))) {
                    return cid != null ? cid : generateMockAndroidId();
                }
                return result;
            } catch (Exception e) {
