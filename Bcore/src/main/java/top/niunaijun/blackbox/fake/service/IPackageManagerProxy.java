...
public class IPackageManagerProxy extends BinderInvocationStub {
    ...
    public static class GetProviderInfo extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            // (black screen). Try resolving the same provider by its androidx-startup authority.
            if (componentName != null && componentName.getPackageName() != null) {
                try {
                    int f = flags | android.content.pm.PackageManager.GET_META_DATA;
                    String auth = componentName.getPackageName() + ".androidx-startup";
                    ProviderInfo byAuth = BlackBoxCore.getBPackageManager()
                            .resolveContentProvider(auth, f, BlackBoxCore.getUserId());
                    if (byAuth != null) {
                        if (componentName.getClassName().equals(byAuth.name)) {
                            return byAuth;
                        }
                        // androidx App Startup hardcodes a query for the BASE class name
                        // (androidx.startup.InitializationProvider) even when the app registers a
                        // SUBCLASS on the androidx-startup authority (e.g. Instagram's
                        // AndroidXAppInitializer). Return it under the requested name so the app
                        // gets the initializer meta-data instead of a NameNotFound crash.
                        if ("androidx.startup.InitializationProvider".equals(componentName.getClassName())) {
                            ProviderInfo pi = new ProviderInfo(byAuth);
                            pi.name = componentName.getClassName();
                            return pi;
                        }
                    }
                } catch (Throwable t) {
                    Slog.w(TAG, "provider authority fallback failed: " + t);
                }
            }
