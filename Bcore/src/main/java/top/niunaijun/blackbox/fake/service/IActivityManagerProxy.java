...
public class IActivityManagerProxy extends ClassInvocationStub {
    ...
    public static Object BindServiceCommon(Object who, Method method, Object[] args,int callingPackageIndex) throws Throwable {
        try {
            ...
            // Advertising-ID service usually resolves to the REAL system GMS (not the
            // container), which leaks a shared GAID. Wrap the connection with our delegate
            // so the per-clone GAID stub is swapped in when it connects, then let the real
            // bind proceed.
            if (resolveInfo == null && connection != null
                    && ServiceConnectionDelegate.isAdIdBind(intent)
                    && !AppSystemEnv.isOpenPackage(intent.getComponent())) {
